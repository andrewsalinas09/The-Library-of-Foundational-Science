...
The table above is twelve ticks of exactly this arithmetic, frozen. Here it is running. Drag the input and watch the loop re-balance: the bucket never settles, the comparator keeps flipping, and the density of ones tracks a value no single comparison could ever have reported.
